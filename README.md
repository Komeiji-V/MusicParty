...
> ## ⚡ 本分支改造说明（new-MusicParty）
>
> 在原版基础上完成了以下升级：
> - **统一登录**：接入自研统一认证中心（`AUTH_CENTER_URL`），账号全局通用，频道内用户名与认证中心一致；认证中心 `admin` 角色自动成为本站总管理员
> - **多频道系统**：类似 TeamSpeak 的频道概念，顶部下拉快速切换；每频道独立队列/播放器/聊天，互不干扰
> - **频道权限四模式**：公开 / 密码 / 邀请制（成员白名单）/ 仅成员可见（对标 TeamSpeak 用户管理）
> - **PostgreSQL 持久化**：队列/聊天/历史/缓存/点赞全部落库，重启可恢复
> - **个人歌单**：分类管理、快速点歌、一键收藏；支持导出为各平台可导入的 TXT（`歌名 - 歌手`）+ JSON 汇总
> - **公开 API**：频道状态/在线人数/正在播放/点赞数等，附 iframe 嵌入卡片页（`/embed/channel.html?id=x`）
> - **可配置定时清理**：管理后台设定间隔/保留天数/清理目标，自动清理防数据库膨胀
> - **站点品牌**：标题/作者/信息页（HTML）后台可配置
>
> **部署请参阅 [docs/DEPLOY.md](docs/DEPLOY.md)（含认证中心对接、环境变量、数据库迁移）。**
