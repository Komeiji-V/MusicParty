<template>
  <div class="h-24 bg-white border-t border-medical-200 flex items-center px-4 md:px-8 relative z-50 shadow-lg">
    <!-- 收藏到歌单选择器 -->
    <PlaylistPicker v-if="pickerMusic" :music="pickerMusic" @close="pickerMusic = null" />
    <!-- 封面 -->
    <div
        id="tutorial-source"
    ...
    <div class="flex-1 ml-4 mr-4 md:mr-8 flex flex-col justify-center min-w-0">
      ...
      <div class="flex md:hidden justify-end gap-3 mt-2">
        ...
        <button @click="pickerMusic = nowPlaying?.music" :disabled="!nowPlaying" class="p-2 bg-medical-50 border border-medical-200 rounded-sm text-medical-500 active:bg-medical-200 disabled:opacity-30">
        ...
        >
         ...
         >
             ...
             <!-- Badge -->
             <div v-if="player.isVoteSkipEnabled && canVote && player.currentVotes > 0" class="absolute -top-1 -right-1 bg-accent text-white text-[10px] font-bold px-1 min-w-[12px] h-3 flex items-center justify-center rounded-sm shadow-sm">
                {{ player.currentVotes }}
             </div>
         </button>
      </div>
    ...
    <div class="hidden md:flex items-center gap-6 flex-shrink-0">
      ...
      <div class="flex items-center gap-4 border-r border-medical-200 pr-6">
        ...
        <button @click="pickerMusic = nowPlaying?.music" :disabled="!nowPlaying" title="收藏到歌单" class="text-medical-400 hover:text-accent transition-colors disabled:opacity-30 disabled:cursor-not-allowed">
        ...
        >
            ...
            <!-- Badge -->
            <div v-if="player.isVoteSkipEnabled && canVote && player.currentVotes > 0" class="absolute -top-1 -right-1 bg-accent text-white text-xs font-bold px-1 min-w-[14px] h-3.5 flex items-center justify-center rounded-sm shadow-sm">
                {{ player.currentVotes }}
            </div>
            <!-- Wait Timer Hint -->
            <div v-if="player.isVoteSkipEnabled && canVote && waitTimeLeft > 0" class="absolute -bottom-6 left-1/2 -translate-x-1/2 bg-medical-900 text-white text-[10px] px-1 py-0.5 rounded-sm opacity-0 group-hover/skip:opacity-100 transition-opacity whitespace-nowrap">
                {{ waitTimeLeft }}s 后可投票
            </div>
        </button>
      </div>
      ...
      <div class="flex items-center gap-2 group">
        ...
        <div class="w-8 text-xs font-mono text-medical-400 text-right">
...
import { Download, ListOrdered, Repeat1, Shuffle, SkipForward, Play, Pause, Volume2, Volume1, VolumeX, ExternalLink, Zap, Lock, BookmarkPlus } from 'lucide-vue-next';
