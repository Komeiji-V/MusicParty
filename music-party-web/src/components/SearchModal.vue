          ...
          <div class="flex-1 overflow-y-auto p-2 space-y-2">
            ...
            <template v-else>
              <div class="flex justify-between items-center px-2 py-1 bg-medical-50 border-b border-medical-100">
                <span class="text-xs font-mono text-medical-400">ID: {{ bindings[platform] }}</span>
                <button @click="playerStore.bindAccount(platform, '')" class="text-xs text-red-400 hover:underline">UNLINK</button>
              </div>
          ...
          <div @scroll="handleScroll" class="flex-1 overflow-y-auto p-2 md:p-4">
            ...
            <div class="space-y-1">
              ...
              <div v-for="song in songs" :key="song.id" class="flex items-center p-3 border border-transparent transition-all group" :class="isUnplayable(song) ? 'opacity-50 grayscale bg-medical-50 cursor-not-allowed' : 'bg-white hover:border-medical-300 hover:shadow-sm'">
                ...
                <div v-if="isUnplayable(song)" class="ml-2 flex-shrink-0"><span class="px-1.5 py-0.5 text-xs font-mono font-bold text-medical-400 border border-medical-300 bg-medical-100 rounded-sm">>10MIN</span></div>

                <button
                    v-if="!isUnplayable(song)"
                    @click="pickerMusic = { ...song, platform: platform }"
                    title="收藏到歌单"
                    class="ml-2 p-2 flex-shrink-0 text-medical-300 hover:text-accent transition-all duration-300"
                >
...
const isPlatformEnabled = (p) => {
  ...
  if (p === 'kugou') return playerStore.config?.kugouEnabled !== false;
  return true;
};
