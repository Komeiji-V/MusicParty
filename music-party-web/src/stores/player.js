...
export const usePlayerStore = defineStore('player', () => {
    ...
    const connect = () => {
        ...
        // 使用抽离出的订阅配置（按频道动态 topic）
        const channelId = localStorage.getItem(STORAGE_KEYS.CHANNEL_ID) || '1';
        const subscriptions = createSocketSubscriptions(channelId);
    ...
    // 切换频道：断开当前 WebSocket 并携带新 channel-id 重新连接
    const reconnectForChannel = () => {
    ...
    // 清空自己点播（原 //clear 命令的可视化）
    const sendClearMine = () => requireAuth() && socketService.send(WS_DEST.QUEUE_CLEAR_MINE);
    ...
    return {
        ...
        enqueue, enqueuePlaylist, topSong, removeSong, sendClearMine,
